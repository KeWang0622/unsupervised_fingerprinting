...
if options.double:
    t_color = np.concatenate((t1_list/t1_list.max(),t2_list/t2_list.max(),np.zeros(t1_list.shape)),1)
    ...
    p = ax.scatter(dic_compressed[:,0], dic_compressed[:,1], dic_compressed[:,2], c=t_color,marker = '*',linewidths=0.3,cmap = "hot")
    ax.set_xlim([-1, 1])
    ax.set_zlim([-1,1])
    ax.set_ylim([-1, 1])
    # fig.colorbar(p)
